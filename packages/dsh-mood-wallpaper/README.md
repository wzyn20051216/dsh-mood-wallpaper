...
| 🚀 **自适应性能档位** | 自动检测平均 FPS → 60/30/15 分档；电池供电降着色器分辨率；高上下文压力/大量工具调用减非关键特效；WebGL 上下文丢失自动恢复；设置页展示 GPU/CPU 开销 |
| 🌌 **记忆星图** | 壁纸层记忆可视化：问题=恒星、决策/工具/注入=轨道节点、错误=红色脉冲、固定记忆=金色星座，点击节点查看详情 |
| 🎨 **场景编排器 Scene Studio** | 可视化编排壁纸/着色器/桌宠/音效/HUD 布局 + idle/thinking/tool/approval/error/done 状态预览 + scene.json 导出/导入 |
| 🛰️ **多 Agent 任务现场** | 后台任务=光点舰船：运行沿轨道、工具调用连线、等待批准琥珀停靠、完成返回中心、失败故障波纹 |
