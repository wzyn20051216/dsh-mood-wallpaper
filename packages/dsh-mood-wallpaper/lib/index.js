...
/** 导入白名单：不含 svg（同源 XSS 风险），也不含未知类型。 */
const IMPORT_EXT_RE = /\.(png|jpe?g|webp|gif|bmp|avif|apng|mp4|webm)$/i;

/** 魔数校验：防止伪装扩展名落盘 HTML/JS。 */
function sniff(ext, b) {
  if (b.length < 8) return false;
  const eq = (off, arr) => { for (let i = 0; i < arr.length; i++) if (b[off + i] !== arr[i]) return false; return true; };
  if (ext === "gif") return eq(0, [0x47, 0x49, 0x46]); // GIF8
  if (ext === "png" || ext === "apng") return eq(0, [0x89, 0x50, 0x4E, 0x47]);
  if (ext === "jpg" || ext === "jpeg") return eq(0, [0xFF, 0xD8]);
  if (ext === "webp") return eq(0, [0x52, 0x49, 0x46, 0x46]); // RIFF
  if (ext === "bmp") return eq(0, [0x42, 0x4D]); // BM
  if (ext === "mp4" || ext === "avif") return eq(4, [0x66, 0x74, 0x79, 0x70]); // ftyp
  if (ext === "webm") return eq(0, [0x1A, 0x45, 0xDF, 0xA3]); // EBML
  return false;
}
...
/** 名称清洗：仅允许文件名，禁止路径穿越；畸形编码返回 null。 */
function safeName(raw) {
  if (typeof raw !== "string" || raw.length === 0) return null;
  let base;
  try {
    base = basename(decodeURIComponent(raw));
  } catch {
    return null;
  }
...
async function handleImport(req, res, query) {
  ...
  if (id === null || !IMPORT_EXT_RE.test(id)) {
    sendJson(res, 400, { ok: false, error: "文件名不合法（支持图片/GIF/APNG/MP4/WebM，不支持 SVG）" });
    return;
  }
  try {
    ...
    const ext = id.split(".").pop().toLowerCase();
    if (!sniff(ext, body)) {
      sendJson(res, 400, { ok: false, error: "文件内容与扩展名不匹配（已拒绝）" });
      return;
    }
    const dir = storageDir();
