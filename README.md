...
| 类别 | 能力 |
|---|---|
| 🖼️ **壁纸** | 导入动态壁纸（GIF/视频/APNG/SVG/静态，≤48MB）+ **自定义壁纸文件夹**（任意目录，壁纸放非系统盘不占 C 盘）+ 3 张内置程序化动态壁纸 + 3 张 **WebGL 实时着色器壁纸**（极光/熔岩/星云，60fps 零依赖） |
...
- **壁纸文件夹**：设置一个本地目录（如 `E:\wallpapers`），目录里所有图片/GIF/视频自动出现在壁纸列表（带「文件夹」徽标）——大量壁纸建议放非系统盘，不占 C 盘
